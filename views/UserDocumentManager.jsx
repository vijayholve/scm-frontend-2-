...
const UserDocumentManager = ({ userId, userType }) => {
 ...
 const fetchDocuments = useCallback(async () => {
  setLoading(true);
  try {
   // Correct API endpoint and payload for fetching all documents by user ID
    const payload = {
      page: 0,
      size: 1000,
      sortBy: 'createdDate',
   ...
   const response = await api.post(`/api/documents/getAllBy/${accountId}`, payload);
           ...
           <TableCell>
            <Tooltip title="Download">
             <IconButton color="info" onClick={async () =>{
              ...
              const res = await api.get(`/api/documents/download/${accountId}/${doc.id}`, {
                method: 'GET',
                headers: {},
        ...
        <CardContent>
         ...
         {newDocument.file && (
          <Chip
           ...
           onDelete={() => setNewDocument({ ...newDocument, file: null })}
           sx={{ ml: 2 }}
          />
         )}
         {uploadError && (
