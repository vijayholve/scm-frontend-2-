    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1, mb: 0.5 }}>
          ...
          <Typography variant="h6" sx={{ fontSize: '14px', fontWeight: 700, letterSpacing: '0.5px' }}>
            {t('template.info.schoolName') || 'SUNSHINE HIGH SCHOOL'}
          </Typography>
        </Box>
        <Typography variant="caption" sx={{ fontSize: '10px', opacity: 0.9 }}>
          {t('template.info.schoolSlogan') || 'Excellence in Education'}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', p: 1.5, gap: 1.5 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Typography variant="caption" sx={{ mt: 0.5, fontSize: '9px', opacity: 0.8 }}>
            {isStudent ? t('template.common.student') || 'STUDENT' : t('template.common.faculty') || 'FACULTY'}
          </Typography>
        </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.3, fontSize: '10px' }}>
            {isStudent && data.rollNo && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <Typography variant="caption" sx={{ fontSize: '10px' }}>
                  {(t('template.common.label.rollNo') || 'Roll No') + ':'} {data.rollNo}
                </Typography>
              </Box>
            )}
            ...
            {isStudent && (data.className || data.divisionName) && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <Typography variant="caption" sx={{ fontSize: '10px' }}>
                  {t('template.common.label.class') || 'Class'}: {data.className} - {data.divisionName}
                </Typography>
              </Box>
            )}
            ...
            {!isStudent && data.role?.roleName && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <BadgeIcon sx={{ fontSize: 12, opacity: 0.8 }} />
                <Typography variant="caption" sx={{ fontSize: '10px' }}>
                  {t('template.common.label.role') || 'Role'}: {data.role.roleName}
                </Typography>
              </Box>
            )}
            ...
            {data.dob && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <Typography variant="caption" sx={{ fontSize: '10px' }}>
                  {t('template.common.label.dob') || 'DOB'}: {new Date(data.dob).toLocaleDateString()}
                </Typography>
              </Box>
            )}
      ...
      >
        <Typography variant="caption" sx={{ fontSize: '8px', opacity: 0.9 }}>
          {t('template.info.classicSlogan') || '"Empowering Education, Building Future"'}
        </Typography>
      </Box>
    </IdCardContainer>
  );
    ...
    >
      ...
      >
        ...
        <Typography variant="caption" sx={{ fontSize: '10px', color: '#7f8c8d', fontStyle: 'italic' }}>
          {t('template.info.classicSlogan') || 'Est. 1985 • Excellence in Education'}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', p: 1.5, gap: 1.5 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Box sx={{ mt: 0.5, px: 1, py: 0.2, backgroundColor: '#34495e', borderRadius: '10px' }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.3 }}>
            {isStudent && data.rollNo && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Typography variant="caption" sx={{ fontSize: '10px', fontWeight: 600, color: '#e74c3c' }}>
                  {t('template.common.label.rollNo') || 'Roll No'}:
                </Typography>
            ...
            {isStudent && (data.className || data.divisionName) && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Typography variant="caption" sx={{ fontSize: '10px', fontWeight: 600, color: '#e74c3c' }}>
                  {t('template.common.label.class') || 'Class'}:
                </Typography>
                <Typography variant="caption" sx={{ fontSize: '10px', color: '#2c3e50' }}>
            ...
            {!isStudent && data.role?.roleName && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Typography variant="caption" sx={{ fontSize: '10px', fontWeight: 600, color: '#e74c3c' }}>
                  {t('template.common.label.role') || 'Role'}:
                </Typography>
                <Typography variant="caption" sx={{ fontSize: '10px', color: '#2c3e50' }}>
                  {data.role.roleName}
                </Typography>
            ...
            {data.dob && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <Typography variant="caption" sx={{ fontSize: '10px', fontWeight: 600, color: '#e74c3c' }}>
                  {t('template.common.label.dob') || 'DOB'}:
                </Typography>
                <Typography variant="caption" sx={{ fontSize: '10px', color: '#2c3e50' }}>
      ...
      >
        <Typography variant="caption" sx={{ fontSize: '8px', color: '#7f8c8d', fontWeight: 500 }}>
          {t('template.info.minimalSlogan') || 'www.sunshineschool.edu • info@sunshineschool.edu'}
        </Typography>
      </Box>
    </IdCardContainer>
  );
  ...
  if (!data) {
    return (
      <IdCardContainer sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Typography color="text.secondary">{t('modal.noData') || 'No data available'}</Typography>
      </IdCardContainer>
    );
  }
