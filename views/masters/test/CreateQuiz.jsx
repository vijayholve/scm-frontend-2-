...
// SCD selector + central SCD hook (do not fetch school/class/division locally)
...
const CreateQuiz = () => {
  ...
  // SCD data is provided centrally; do NOT call school/class/division APIs locally
  ...
  const addOrUpdateQuestion = () => {
    ...
    if (editQuestionIndex !== null) {
      ...
      setQuiz((prev) => ({
        ...prev,
        questions: prev.questions.map((q, idx) => (idx === editQuestionIndex ? newQuestion : q))
      }));
  ...
  const editQuestion = (index) => {
    ...
    setCurrentQuestion({
      ...question,
      options:
        question.type === 'PARAGRAPH' ? ['', '', '', ''] : [...question.options, ...Array(4 - question.options.length).fill('')].slice(0, 4)
    });
  ...
  const scdAdapter = {
    values: { schoolId: quiz.schoolId, classId: quiz.classId, divisionId: quiz.divisionId },
    setFieldValue: (field, value) => {
      if (field === 'schoolId') {
        ...
        setQuiz((prev) => ({ ...prev, classId: value ?? null, className: cls?.name ?? '' }));
      } else if (field === 'divisionId') {
        const div = (divisions || []).find((d) => String(d.id) === String(value));
        setQuiz((prev) => ({ ...prev, divisionId: value ?? null, divisionName: div?.name ?? '' }));
      } else {
        ...
        >
          <Grid container spacing={gridSpacing} sx={{ mt: 2 }}>
                          ...
                          <CardContent>
                            ...
                            >
                              <Box flex={1}>
                                ...
                                <Chip label={questionTypes.find((t) => t.value === question.type)?.label} size="small" color="primary" />
                              ...
                              >
                                <IconButton color="primary" onClick={() => editQuestion(index)} sx={{ mr: 1 }}>
                                  <SaveIcon />
                                </IconButton>
            ...
            <Grid item xs={12}>
              ...
              >
                <Button variant="outlined" onClick={() => navigate('/masters/quiz')}>
                  Cancel
                </Button>
