...
import {
  ...
  CreditCard as LeavingCertificateIcon,
...
// modal lives in sibling folder 'leaving' (not 'Leavings')
import LeavingCertificateGenerationModal from '../Leavings/LeavingCertificateGenerationModal.jsx';
...
function TabPanel(props) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`leavingCertificate-tabpanel-${index}`}
      aria-labelledby={`leavingCertificate-tab-${index}`}
      {...other}
    >
...
function a11yProps(index) {
  return {
    id: `leavingCertificate-tab-${index}`,
    'aria-controls': `leavingCertificate-tabpanel-${index}`
  };
}

const LeavingCertificateManagement = () => {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <IconButton
            ...
            onClick={() => handleSingleLeavingCertificate(params.row)}
          >
            <LeavingCertificateIcon fontSize="small" />
          </IconButton>
        </Box>
      ),
      disableColumnMenu: true,
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <IconButton size="small" color="info" title="Generate ID Card" onClick={() => handleSingleLeavingCertificate(params.row)}>
            <LeavingCertificateIcon fontSize="small" />
          </IconButton>
        </Box>
      ),
  ...
  const handleSingleLeavingCertificate = (userData) => {
    setSelectedRows([userData.id]);
    setGenerationModalOpen(true);
  };

  const handleBulkLeavingCertificate = () => {
  ...
  return (
    ...
    >
          ...
          <ActionContainer>
            <Grid container spacing={2} alignItems="center" justifyContent="flex-end">
              ...
              <Grid item>
                <Button
                  variant="contained"
                  startIcon={<LeavingCertificateIcon />}
                  onClick={handleBulkLeavingCertificate}
      ...
      <LeavingCertificateGenerationModal
...
export default LeavingCertificateManagement;
